...
namespace CarBook.Frontend.Controllers
{
    public class AdminBrandController : Controller
    {
        ...
        public async Task<IActionResult> CreateBrand(CreateBrandDto createBrandDto)
        {
            ...
            var response = await client.PostAsync("http://localhost:7070/api/Brand", stringContent);
            if (response.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMeesage = await client.GetAsync($"http://localhost:7070/api/Brand/{id}");
        ...
        public async Task<IActionResult> UpdateBrand(UpdateBrandDto updateBrandDto,string id)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, encoding: Encoding.UTF8, "application/json");
            var response = await client.PutAsync("http://localhost:7070/api/Brand", stringContent);
            if (response.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "AdminBrand");
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var response = await client.DeleteAsync($"http://localhost:7070/api/Brand?id={id}");
            if (response.IsSuccessStatusCode)
            {
